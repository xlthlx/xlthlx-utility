<?php
/**
 * Template and rewrite functions for English translation.
 *
...
function xlt_en_title( $title ) {
	...
	if ( 'en' === $lang ) {
		...
		if ( is_month() ) {
			...
			$title    = gmdate( 'F', strtotime( $datetime ) ) . ' ' . get_the_time( 'Y' ) . ' | ' . get_option( 'english_title', '' );
		}
	}
...
function xlt_search_url_rewrite() {
	...
	if ( strpos( $uri, $needle ) === false && strpos( $uri, '&lang=en' ) !== false ) {
		...
		wp_safe_redirect( home_url() . '/search/' . $search . '/en/' );
		exit();
	}
