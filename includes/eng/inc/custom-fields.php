 ...
 * @return void
 */
function xlt_add_category_metabox(): void {
 ...
 */
function xlt_eng_posts_columns( array $defaults ): array {
...
function xlt_eng_posts_custom_columns( string $column_name, int $id ): void {
	...
	if ( 'comments-number' === $column_name ) {
		...
		if ( '' !== $comments_number ) {
			echo '<a title="Visualizza commenti" href="/wp-admin/edit-comments.php?p=' . esc_attr( $id ) . '&comment_status=approved">' . esc_attr( $comments_number ) . ' &raquo;</a>';
		}
		if ( ( '' !== $comments_number ) && ( '' !== $comments_open ) ) {
			echo '<br/>';
		}
		if ( '' !== $comments_open ) {
			echo esc_attr( $comments_open );
		}
		if ( ( '' !== $comments_open ) && ( '' !== $pings_open ) ) {
			echo '<br/>';
		}
		if ( '' !== $pings_open ) {
			echo esc_attr( $pings_open );
		}
	}
}
 ...
 * @return void
 */
function xlt_register_comment_language(): void {
