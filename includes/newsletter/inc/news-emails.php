 ...
 * @return string
 */
function xlt_trim( string $text ): string {
 ...
 */
function xlt_send_confirmation( string $lang, string $to, string $_code ): void {
	...
	$include    = XLT_PLUGIN_PATH . '/includes/newsletter/inc/email/confirm-' . $lang . '.php';
	include $include;
	$body = ob_get_clean();
 ...
 */
function xlt_post_published_notification( $post_id, $post, $old_status ): void {
		...
		if ( $query->have_posts() ) {
			...
			while ( $query->have_posts() ) {
				...
				if ( 'en' === $_lang ) {
					...
					$excerpt    = $_excerpt_en;
					include XLT_PLUGIN_PATH . 'includes/newsletter/inc/email/post-en.php';
				} else {
					$site_title = $site_name;
					...
					$excerpt    = $_excerpt;
					include XLT_PLUGIN_PATH . 'includes/newsletter/inc/email/post-it.php';
				}
				$body = ob_get_clean();
...
add_action( 'wp_mail_failed', 'xlt_log_mail_error' );
