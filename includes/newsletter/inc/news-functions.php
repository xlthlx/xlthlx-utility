 ...
 * @return void
 */
function xlt_remove_flamingo_save(): void {
 ...
 * @return void
 */
function xlt_flamingo_submit( object $contact_form, array $result ): void {
	if ( ! class_exists( 'Flamingo_Contact' ) || ! class_exists( 'Flamingo_Inbound_Message' ) ) {
		return;
	}

	if ( ! class_exists( 'WPCF7_Submission' ) || ! class_exists( 'WPCF7_MailTag' ) ) {
		return;
	}
	...
	if ( empty( $result['status'] ) || ! in_array( $result['status'], $cases, true ) ) {
		return;
	}
	...
	$channel_add_id = function_exists( 'wpcf7_flamingo_add_channel' ) ? wpcf7_flamingo_add_channel(
		$contact_form->name(),
		$contact_form->title()
	) : 0;
	$channel_id     = isset( $post_meta['channel'] ) ? (int) $post_meta['channel'] : $channel_add_id;
 ...
 * @return array
 */
function xlt_newsletter_query_vars( array $vars ): array {
	$vars[] .= 'act';
	$vars[] .= 'cod';
